...
def fetch_weather(request):
    if request.method == "POST":
        ...
        api_key = settings.OPENWEATHER_API_KEY  # Get the API key from your settings file
        ...
        if response.status_code == 200:
            ...
            CityWeather.objects.create(city_name=city_name, temperature=temperature, description=description)

            return render(request, 'weather_result.html', {'weather': data})
        else:
            return render(request, 'weather_result.html', {'error': 'City not found!'})
